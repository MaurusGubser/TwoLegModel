...
class Plotter:
    def __init__(self, true_states, true_obs, delta_t, export_name, show_fig):
        ...
        if nb_steps_states != nb_steps_obs:
            raise AssertionError(
                'States and observation have different number of steps: {} and {}, respectively'.format(
                    nb_steps_states,
                    nb_steps_obs))
    ...
    def plot_particles_trajectories(self, X_hist):
        ...
        if self.dim_states == 18:
            state_names = ['$x_H$', '$y_H$', r'$\varphi_0$', r'$\varphi_1$',
                           r'$\varphi_2$', r'$\varphi_3$',
                           r'$\dot x_H$', r'$\dot y_H$', r'$\dot \varphi_0$',
                           r'$\dot \varphi_1$', r'$\dot \varphi_2$',
                           r'$\dot \varphi_3$',
                           r'$\ddot x_H$', r'$\ddot y_H$', r'$\ddot \varphi_0$',
                           r'$\ddot \varphi_1$',
                           r'$\ddot \varphi_2$',
                           r'$\ddot \varphi_3$']
            ...
            state_names = ['$x_H$', '$y_H$', r'$\varphi_0$', r'$\varphi_1$',
                           ...
                           r'$\ddot x_H$', r'$\ddot y_H$', r'$\ddot \varphi_0$',
                           r'$\ddot \varphi_1$']
            self.true_states = self.true_states[:,
                               (0, 1, 2, 3, 6, 7, 8, 9, 12, 13, 14, 15)]
        else:
            raise AssertionError(
                'Dimension of state vector expected to be 12 or 18; got {}'.format(
                    self.dim_states))
        ...
        for i in range(0, nb_figures):
            ...
            for j in range(0, nb_axes):
                ...
                for k in range(0, nb_graphs):
                    axs[j].plot(self.t_vals, X_hist[:, k, nb_axes * i + j],
                                label='Sample {}'.format(k), lw=1)
                axs[j].plot(self.t_vals,
                            self.true_states[:, :, nb_axes * i + j],
                            label='truth', lw=1.5, color='green')
                axs[j].set_title(state_names[nb_axes * i + j])
                axs[j].legend()
                if state_names[nb_axes * i + j] == r'$y_H$':
                    axs[j].plot(self.t_vals, self.contact_left + 1.0,
                                label='Contact left', color='red', lw=1.5)
                    axs[j].plot(self.t_vals, self.contact_right + 1.0,
                                label='Contact right', color='orange', lw=1.5)
                elif state_names[nb_axes * i + j] == r'$\dot x_H$':
                    axs[j].plot(self.t_vals, self.contact_left + 0.6,
                                label='Contact left', color='red', lw=1.5)
                    axs[j].plot(self.t_vals, self.contact_right + 0.6,
                                label='Contact right', color='orange', lw=1.5)
                else:
                    axs[j].plot(self.t_vals, self.contact_left,
                                label='Contact left', color='red', lw=1.5)
                    axs[j].plot(self.t_vals, self.contact_right,
                                label='Contact right', color='orange', lw=1.5)
            ...
            if self.export_path is not None:
                plt.savefig(self.export_path + '/ParticleTrajectories_' + str(
                    i) + '.pdf')
        if self.show_fig:
            plt.show()
        return None
    ...
    def plot_smoothed_trajectories(self, samples):
        ...
        for i in range(0, nb_figures):
            ...
            for j in range(0, nb_axes):
                ...
                axs[j].grid(axis='both')
                for k in range(0, nb_graphs):
                    axs[j % nb_axes].plot(self.t_vals,
                                          samples[:, k, nb_axes * i + j],
            ...
            axs_list.append(axs)
            if self.export_path is not None:
                plt.savefig(self.export_path + '/SmoothedTrajectories_' + str(
                    i) + '.pdf')
        if self.show_fig:
            plt.show()
        return None
        ...
        for i in range(0, nb_figures):
            ...
            for j in range(0, nb_axes):
                if i * nb_axes + j > self.dim_states - 1:
                    break
                axs[j].grid(axis='both')
                axs[j].plot(self.t_vals, particles_mean[:, nb_axes * i + j],
                            label='Particle mean', color='blue')
                axs[j].fill_between(self.t_vals, particles_mean[:,
                                                 nb_axes * i + j] - particles_sd[
                                                                    :,
                                                                    nb_axes * i + j],
                                    particles_mean[:,
                                    nb_axes * i + j] + particles_sd[:,
                ...
                axs[j].plot(self.t_vals,
                            self.true_states[:, :, nb_axes * i + j],
                            label='Truth', color='green', lw=1.5)
                if state_names[nb_axes * i + j] == r'$y_H$':
                    axs[j].plot(self.t_vals, self.contact_left + 1.0,
                                label='Contact left', color='red', lw=1.5)
                    axs[j].plot(self.t_vals, self.contact_right + 1.0,
            ...
            axs_list.append(axs)
            if self.export_path is not None:
                plt.savefig(self.export_path + '/MomentsTrajectories_' + str(
                    i) + name_suffix + '.pdf')
        if self.show_fig:
            plt.show()
        return None
    ...
    def plot_observations(self, samples, model):
        ...
        if self.dim_observations == 20:
            obs_names = ['$\ddot x^0$', '$\ddot y^0$', '$\omega_z^0$',
                         ...
                         '$\dot x^4$', '$\dot y^4$', '$\ddot x^4$',
                         '$\ddot y^4$',
                         '$\dot x^5$', '$\dot y^5$', '$\ddot x^5$',
                         '$\ddot y^5$']
        elif self.dim_observations == 36:
            obs_names = ['$\ddot x^0$', '$\ddot y^0$', '$\ddot z^0$',
                         '$\omega_x^0$', '$\omega_y^0$', '$\omega_z^0$',
                         '$\ddot x^1$', '$\ddot y^1$', '$\ddot z^1$',
                         '$\omega_x^1$', '$\omega_y^1$', '$\omega_z^1$',
                         '$\ddot x^2$', '$\ddot y^2$', '$\ddot z^2$',
                         '$\omega_x^2$', '$\omega_y^2$', '$\omega_z^2$',
                         '$\ddot x^3$', '$\ddot y^3$', '$\ddot z^3$',
                         '$\omega_x^3$', '$\omega_y^3$', '$\omega_z^3$',
                         '$\dot x^4$', '$\dot y^4$', '$\dot z^4$',
                         '$\ddot x^4$', '$\ddot y^4$', '$\ddot z^4$',
                         '$\dot x^5$', '$\dot y^5$', '$\dot z^5$',
                         '$\ddot x^5$', '$\ddot y^5$', '$\ddot z^5$']
        else:
            raise ValueError(
                'Observation dimension has to be 20 or 36; got {} instead.'.format(
                    self.dim_observations))
        ...
        for i in range(0, nb_figures):
            ...
            for j in range(0, nb_axes):
                ...
                axs[j].grid(axis='both')
                for k in range(0, nb_graphs):
                    axs[j].plot(self.t_vals, obs[:, k, i * nb_axes + j],
                                label='Sample {}'.format(k), lw=1)
                axs[j].plot(self.t_vals, true_obs[:, :, i * nb_axes + j],
                            label='True observation', color='green',
                            lw=1.5)
                if obs_names[i * nb_axes + j] in ['$\ddot y^0$', '$\ddot y^1$',
                                                  '$\ddot y^2$', '$\ddot y^3$']:
                    axs[j].plot(self.t_vals, self.contact_left + 10.0,
                                label='Contact left', color='red', lw=1.5)
                    axs[j].plot(self.t_vals, self.contact_right + 10.0,
                                label='Contact right', color='orange', lw=1.5)
                else:
                    axs[j].plot(self.t_vals, self.contact_left,
                                label='Contact left', color='red', lw=1.5)
            ...
            if self.export_path is not None:
                plt.savefig(
                    self.export_path + '/ObservationTrajectories_' + str(
                        i) + '.pdf')
        if self.show_fig:
            plt.show()
        return None
    ...
    def plot_ESS(self, ESS):
        ...
        plt.plot(self.t_vals, self.contact_left, label='Contact left',
                 color='red', lw=1.5)
        plt.plot(self.t_vals, self.contact_right, label='Contact right',
                 color='orange', lw=1.5)
    ...
    def plot_logLts_multiple_runs(self, output_multismc, nb_particles, nb_runs,
                                  t_start):
        logLts_data = {}
        for N in nb_particles:
            logLts_data[N] = np.array(
                [r['output'].summaries.logLts for r in output_multismc if
                 r['N'] == N])
        ...
        for key, loglts in logLts_data.items():
            ...
            fig.suptitle(
                'Loglikelihood, non-truncated, averaged over {} runs'.format(
                    nb_runs))
            if self.export_path:
        ...
        for key, loglts in logLts_data.items():
            logLts_truncated = np.array(
                [loglts[:, -1] - loglts[:, t] for t in t_start_vals]).T
            ...
            axs[0].legend()
            axs[1].plot(t_start_vals, std_trunacted ** 2,
                        label='N={}'.format(key))
            axs[1].set_xlabel('Start time $ t_{0} $')
            axs[1].set_title(
                r'Variance of $ \log(p(y_{t_{0}+1:T} | y_{0:t_{0}})) $')
            axs[1].legend()
        fig.suptitle(
            'Truncated loglikelihood for different starting times, averaged over {} runs'.format(
                nb_runs))
        if self.export_path:
            plt.savefig(
                self.export_path + '/Truncated_Likelihood_different_start_times.pdf')
        ...
        axs[1].grid(axis='both')
        means_N = np.array(
            [np.mean(loglts[:, -1]) for loglts in logLts_data.values()])
        axs[0].plot(nb_particles, means_N, marker='o')
        axs[0].set_xlabel('N')
        axs[0].set_ylabel(r'$\log(p(y_{0:T}))$')
        axs[0].set_title('Loglikelihood')
        means_N_truncated = np.array(
            [np.mean(loglts[:, -1] - loglts[:, t_start]) for loglts in
             logLts_data.values()])
        ...
        fig.suptitle(
            'N vs. Loglikelihood, averaged over {} runs'.format(nb_runs))
        if self.export_path:
        ...
        fig = plt.figure(figsize=(12, 8))
        sb.boxplot(
            x=[r['output'].summaries.logLts[-1] - r['output'].summaries.logLts[
                t_start] for r in output_multismc],
            y=[str(r['N']) for r in output_multismc],
            showfliers=False)
        plt.xlabel('r$\log(p(y_{t_{0}+1:T} | y_{0:t_{0}}))$')
        plt.ylabel('Number of particles')
        fig.suptitle(
            'Truncated loglikelihood of {} timesteps, averaged over {} runs'.format(
                self.nb_steps, nb_runs))
        if self.export_path:
        ...
        fig.suptitle(
            'Histogram of truncated loglikelihood of {} timesteps, averaged over {} runs '.format(
                self.nb_steps,
                nb_runs))
        if self.export_path:
    ...
    def plot_likelihood_parameters(self, output_multismc, model_params,
                                   t_start):
        ...
        logLts_truncated = [
            r['output'].summaries.logLts[-1] - r['output'].summaries.logLts[
                t_start] for r in
            output_multismc]
        plt.figure(figsize=(12, 8))
        ...
        if self.export_path:
            plt.savefig(
                self.export_path + '/Boxplot_truncated_different_params.pdf')
        plt.figure(figsize=(12, 8))
        for fk_model in model_params:
            ...
            print(
                'Parameters={}; mean of loglikelihood={};\nmean of truncated likelihood={}'.format(
                    fk_model, mean[-1],
                    mean[-1] - mean[
                        t_start]))
    ...
    def plot_learned_parameters(self, alg, learning_alg, prior_dict):
        if learning_alg == 'pmmh' or learning_alg == 'cpmmh' or learning_alg == 'gibbs':
            ...
            for i, param in enumerate(prior_dict.keys()):
                ...
                plt.title(param)
                if self.export_path:
                    plt.savefig('{}/Histplot_{}_{}.pdf'.format(self.export_path,
                                                               learning_alg,
                                                               str(param)))
            plt.show()
