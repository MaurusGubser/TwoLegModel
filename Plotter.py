...
class Plotter:
    ...
    def set_contacts(self):
        assert (
            self.dim_observations == 36 or self.dim_observations == 20
        ), "Observation dimension should be 20 or 36; got {} instead".format(
    ...
    def plot_particles_trajectories(self, X_hist):
        ...
        for i in range(0, nb_figures):
            ...
            if self.export_path is not None:
                plt.savefig(
                    self.export_path + "/ParticleTrajectories_" + str(i) + ".pdf"
                )
        if self.show_fig:
            plt.show()
        return None
    ...
    def plot_smoothed_trajectories(self, samples):
        ...
        assert (
            nb_steps == self.nb_steps and dim_states == self.dim_states
        ...
        for i in range(0, nb_figures):
            ...
            if self.export_path is not None:
                plt.savefig(
                    self.export_path + "/SmoothedTrajectories_" + str(i) + ".pdf"
                )
        if self.show_fig:
            plt.show()
        return None
    ...
    def plot_observations(self, samples, model):
        ...
        for i in range(0, nb_figures):
            ...
            for j in range(0, nb_axes):
                ...
                axs[j].plot(
                    self.t_vals,
                    true_obs[:, :, i * nb_axes + j],
                    label="True observation",
                    color="green",
                    lw=1.5,
                )
                if obs_names[i * nb_axes + j] in [
                    "$\ddot y^0$",
                    "$\ddot y^1$",
                    "$\ddot y^2$",
                    "$\ddot y^3$",
            ...
            if self.export_path is not None:
                plt.savefig(
                    self.export_path + "/ObservationTrajectories_" + str(i) + ".pdf"
                )
        if self.show_fig:
            plt.show()
        return None
    ...
    def plot_ESS(self, ESS):
        ...
        plt.plot(
            self.t_vals, self.contact_left, label="Contact left", color="red", lw=1.5
        )
        plt.plot(
            self.t_vals,
            self.contact_right,
    ...
    def plot_logLts_multiple_runs(
        self, output_multismc, nb_particles, nb_runs, t_start
    ):
        logLts_data = {}
        for N in nb_particles:
            logLts_data[N] = np.array(
                [r["output"].summaries.logLts for r in output_multismc if r["N"] == N]
            )
        ...
        for key, loglts in logLts_data.items():
            ...
            fig.suptitle(
                "Loglikelihood, non-truncated, averaged over {} runs".format(nb_runs)
            )
            if self.export_path:
        ...
        for key, loglts in logLts_data.items():
            logLts_truncated = np.array(
                [loglts[:, -1] - loglts[:, t] for t in t_start_vals]
            ).T
        ...
        means_N_truncated = np.array(
            [
                np.mean(loglts[:, -1] - loglts[:, t_start])
                for loglts in logLts_data.values()
            ]
        )
        ...
        sb.boxplot(
            x=[
                r["output"].summaries.logLts[-1] - r["output"].summaries.logLts[t_start]
        ...
        sb.histplot(
            ...
            hue=[str(r["N"]) for r in output_multismc],
            multiple="dodge",
        )
    ...
    def plot_likelihood_parameters(self, output_multismc, model_params, t_start):
        ...
        plt.figure(figsize=(12, 8))
        sb.boxplot(
            x=logLts_truncated, y=[r["fk"] for r in output_multismc], showfliers=False
        )
        ...
        for fk_model in model_params:
            ...
            print(
                "Parameters={}; mean of loglikelihood={};\nmean of truncated likelihood={}".format(
                    fk_model, mean[-1], mean[-1] - mean[t_start]
                )
            )
    ...
    def plot_learned_parameters(self, alg, learning_alg, prior_dict):
        if learning_alg == "pmmh" or learning_alg == "cpmmh" or learning_alg == "gibbs":
            ...
            for i, param in enumerate(prior_dict.keys()):
                ...
                if self.export_path:
                    plt.savefig(
                        "{}/Histplot_{}_{}.pdf".format(
                            self.export_path, learning_alg, str(param)
                        )
                    )
            plt.show()
