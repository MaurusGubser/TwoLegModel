...
class MvNormalMissingObservations(ProbDist):
    """Multivariate Normal distribution with possible missing observations.
    ...
    def logpdf(self, x):
        nb_part, _ = x.shape
        assert nb_part == 1, 'x is expected to be 1-dimensional, got {} array instead'.format(x.shape)
        mask_not_nan = np.invert(np.isnan(x))[0]
        loc_not_nan = self.loc[:, mask_not_nan]
        mask_2d = np.outer(mask_not_nan, mask_not_nan)
        nb_non_nan = np.sum(mask_not_nan)
    ...
    def posterior(self, x, Sigma=None):
        ...
        return MvNormalMissingObservations(loc=mupost, cov=Sigpost)
