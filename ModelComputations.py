...
def state_to_observation_nonlinear(model, x):
    ...
    # left femur
    y[:, 0] = model.cst[0] * x[:, 14] + model.g * np.sin(x[:, 2]) + np.sin(x[:, 2]) * x[:, 13] + np.cos(x[:, 2]) * x[:,
                                                                                                                   12]
    y[:, 1] = model.cst[0] * x[:, 8] ** 2 + model.g * np.cos(x[:, 2]) - np.sin(x[:, 2]) * x[:, 12] + np.cos(
        x[:, 2]) * x[:, 13]
    y[:, 2] = 0.0
    y[:, 3] = 0.0
    y[:, 4] = 0.0
    ...
    # left fibula
    y[:, 6] = model.cst[1] * x[:, 14] + model.cst[1] * x[:, 15] + model.g * np.sin(x[:, 2] + x[:, 3]) + model.legs[
        0] * np.sin(x[:, 3]) * \
              x[:, 8] ** 2 + model.legs[0] * np.cos(x[:, 3]) * x[:, 14] + np.sin(x[:, 2] + x[:, 3]) * x[:, 13] + np.cos(
        x[:, 2] + x[:, 3]) * x[:,
                             12]
    y[:, 7] = model.cst[1] * x[:, 8] ** 2 + 2 * model.cst[1] * x[:, 8] * x[:, 9] + model.cst[1] * x[:,
                                                                                                  9] ** 2 + model.g * np.cos(
        x[:, 2] + x[:, 3]) - model.legs[0] * np.sin(x[:, 3]) * x[:, 14] + model.legs[0] * np.cos(x[:, 3]) * x[:,
                                                                                                            8] ** 2 - np.sin(
    ...
    # right femur
    y[:, 12] = model.cst[2] * x[:, 16] + model.g * np.sin(x[:, 4]) + np.sin(x[:, 4]) * x[:, 13] + np.cos(x[:, 4]) * x[:,
                                                                                                                    12]
    y[:, 13] = model.cst[2] * x[:, 10] ** 2 + model.g * np.cos(x[:, 4]) - np.sin(x[:, 4]) * x[:, 12] + np.cos(
        x[:, 4]) * x[:, 13]
    y[:, 14] = 0.0
    y[:, 15] = 0.0
    y[:, 16] = 0.0
    ...
    # right fibula
    y[:, 18] = model.cst[3] * x[:, 16] + model.cst[3] * x[:, 17] + model.g * np.sin(x[:, 4] + x[:, 5]) + model.legs[
        2] * np.sin(x[:, 5]) * \
               x[:, 10] ** 2 + model.legs[2] * np.cos(x[:, 5]) * x[:, 16] + np.sin(x[:, 4] + x[:, 5]) * x[:,
                                                                                                        13] + np.cos(
        x[:, 4] + x[:, 5]) * \
               x[:, 12]
    y[:, 19] = model.cst[3] * x[:, 10] ** 2 + 2 * model.cst[3] * x[:, 10] * x[:, 11] + model.cst[3] * x[:,
                                                                                                      11] ** 2 + model.g * np.cos(
        x[:, 4] + x[:, 5]) - model.legs[2] * np.sin(x[:, 5]) * x[:, 16] + model.legs[2] * np.cos(x[:, 5]) * x[:,
                                                                                                            10] ** 2 - np.sin(
    ...
    # left heel
    y[:, 24] = model.legs[0] * np.cos(x[:, 2]) * x[:, 8] + model.legs[1] * (x[:, 8] + x[:, 9]) * np.cos(
        x[:, 2] + x[:, 3]) + x[:, 6]
    y[:, 25] = model.legs[0] * np.sin(x[:, 2]) * x[:, 8] + model.legs[1] * (x[:, 8] + x[:, 9]) * np.sin(
        x[:, 2] + x[:, 3]) + x[:, 7]
    y[:, 26] = 0.0
    y[:, 27] = -model.legs[0] * np.sin(x[:, 2]) * x[:, 8] ** 2 + model.legs[0] * np.cos(x[:, 2]) * x[:, 14] - \
               model.legs[1] * (
                       x[:, 8] + x[:, 9]) ** 2 * np.sin(x[:, 2] + x[:, 3]) + model.legs[1] * (
                       x[:, 14] + x[:, 15]) * np.cos(x[:, 2] + x[:, 3]) + x[:,
                                                                          12]
    y[:, 28] = model.legs[0] * np.sin(x[:, 2]) * x[:, 14] + model.legs[0] * np.cos(x[:, 2]) * x[:, 8] ** 2 + model.legs[
        1] * (
                       x[:, 8] + x[:, 9]) ** 2 * np.cos(x[:, 2] + x[:, 3]) + model.legs[1] * (
                       x[:, 14] + x[:, 15]) * np.sin(x[:, 2] + x[:, 3]) + x[:,
                                                                          13]
    y[:, 29] = 0.0

    # right heel
    y[:, 30] = model.legs[2] * np.cos(x[:, 4]) * x[:, 10] + model.legs[3] * (x[:, 10] + x[:, 11]) * np.cos(
        x[:, 4] + x[:, 5]) + x[:, 6]
    y[:, 31] = model.legs[2] * np.sin(x[:, 4]) * x[:, 10] + model.legs[3] * (x[:, 10] + x[:, 11]) * np.sin(
        x[:, 4] + x[:, 5]) + x[:, 7]
    y[:, 32] = 0.0
    y[:, 33] = -model.legs[2] * np.sin(x[:, 4]) * x[:, 10] ** 2 + model.legs[2] * np.cos(x[:, 4]) * x[:, 16] - \
               model.legs[3] * (
                       x[:, 10] + x[:, 11]) ** 2 * np.sin(x[:, 4] + x[:, 5]) + model.legs[3] * (
                       x[:, 16] + x[:, 17]) * np.cos(x[:, 4] + x[:, 5]) + \
               x[:, 12]
    y[:, 34] = model.legs[2] * np.sin(x[:, 4]) * x[:, 16] + model.legs[2] * np.cos(x[:, 4]) * x[:, 10] ** 2 + \
               model.legs[3] * (
                       x[:, 10] + x[:, 11]) ** 2 * np.cos(x[:, 4] + x[:, 5]) + model.legs[3] * (
                       x[:, 16] + x[:, 17]) * np.sin(x[:, 4] + x[:, 5]) + \
               x[:, 13]
    y[:, 35] = 0.0
...
def compute_observation_derivatives_1dim(model, x):
    ...
    # right femur
    df[12, 2] = -x[12] * np.sin(x[4]) + (x[13] + model.g) * np.cos(x[4])
    df[12, 12] = np.cos(x[4])
    df[12, 13] = np.sin(x[4])
    df[12, 14] = model.cst[2]
    df[13, 2] = -x[12] * np.cos(x[4]) - (x[13] + model.g) * np.sin(x[4])
    df[13, 8] = 2 * x[10] * model.cst[2]
    df[13, 12] = -np.sin(x[4])
    df[13, 13] = np.cos(x[4])
    df[17, 10] = 1
    ...
    df[19, 12] = 2 * model.cst[3] * (x[10] + x[11])
    df[19, 13] = -np.sin(x[4] + x[5])
    df[19, 16] = np.cos(x[4] + x[5])
    df[19, 17] = -model.legs[2] * np.sin(x[5])
    df[23, 10] = 1
    df[23, 11] = 1
    ...
    df[25, 3] = x[8] * model.legs[0] * np.cos(x[2]) + model.legs[1] * (x[8] + x[9]) * np.cos(x[2] + x[3])
    df[25, 4] = model.legs[1] * (x[8] + x[9]) * np.cos(x[2] + x[3])
    df[25, 7] = 1
    ...
    df[31, 3] = x[10] * model.legs[2] * np.cos(x[4]) + model.legs[3] * (x[10] + x[11]) * np.cos(x[4] + x[5])
    df[31, 4] = model.legs[3] * (x[10] + x[11]) * np.cos(x[4] + x[5])
    df[31, 7] = 1
...
def plot_observations(model, x, y_true, export_name=None):
    ...
    for i in range(0, nb_figures):
        ...
        for j in range(0, nb_axes):
            ...
            axs[j].plot(t_vals, y_true[:, 3*i + j], label='Truth', ls=':', alpha=0.5, linewidth=2.0)
            axs[j].legend()
