...
class TwoLegModelEKF(ssm.StateSpaceModel):
    ...
    def set_process_cov_state_groups(self):
        sigmas = np.diag(
            [self.sigma_x, self.sigma_y, self.sigma_phi, self.sigma_phi, self.sigma_phi, self.sigma_phi])
    ...
    def state_to_observation(self, x):
        ...
        for i in range(0, nb_samples):
            y[i] = self.state_to_observation_1dim(x[
                                                      i])  # do i have to remove y[i] = ... ? earlier version was self.state_to_observation_1dim(x[i], y[i])
        return y
    ...
    def state_to_observation_1dim(self, x):
        ...
        # left fibula
        y[6] = self.cst[1] * x[14] + self.cst[1] * x[15] + self.g * np.sin(x[2] + x[3]) + self.legs[0] * np.sin(
            x[3]) * \
               x[8] ** 2 + self.legs[0] * np.cos(x[3]) * x[14] + np.sin(x[2] + x[3]) * x[13] + np.cos(x[2] + x[3]) * \
               x[
                   12]
        ...
        # right fibula
        y[18] = self.cst[3] * x[16] + self.cst[3] * x[17] + self.g * np.sin(x[4] + x[5]) + self.legs[2] * np.sin(
            x[5]) * \
                x[10] ** 2 + self.legs[2] * np.cos(x[5]) * x[16] + np.sin(x[4] + x[5]) * x[13] + np.cos(
            x[4] + x[5]) * \
                x[12]
    ...
    def compute_ekf_proposal_1dim(self, xp, data_t, sigma):
    ...
    def PY(self, t, xp, x):
        return dists.MvNormal(loc=self.compute_ekf_proposal(x), cov=self.H)
