...
class DataReaderWriter:
    ...
    def read_observations_as_arr(self, path_observations, max_timesteps):
        ...
        for i, j in pairwise(sensor_idxs):
            ...
            df_sensor.columns = ['time'] + [col_name + '_' + str(i) for col_name
                                            in df_sensor.columns[1:]]
        ...
        for df_obs in sensors_measurements[1:]:
            observation_df = pd.merge(observation_df, df_obs, how='left',
                                      on='time')

        measurement_arr = observation_df.drop('time', axis=1).to_numpy(
            dtype=float)
    ...
    def data_arr_to_list(self):
        if self.observations_arr.shape[0] != self.true_states_arr.shape[0]:
            raise AssertionError(
                'Number of time steps in observations is {}; number of time steps in truth is {};'
        ...
        for time_step in range(0, nb_timesteps):
            self.states_list.append(
                np.reshape(self.true_states_arr[time_step, :], (1, -1)))
            self.observations_list.append(
                np.reshape(self.observations_arr[time_step, :], (1, -1)))
        return None

    def reduce_observations(self):
        self.observations_arr = self.observations_arr[:, (0, 1, 5, 6, 7, 11, 12,
                                                          13, 17, 18, 19, 23,
                                                          24, 25, 27, 28, 30,
                                                          31, 33, 34)]
        return None

    def get_data_as_lists(self, generation_type, max_timesteps,
                          dim_observations):
    ...
    def export_trajectory(data_states, dt, file_name):
        ...
        for i in range(0, nb_samples):
            export_name = 'AnimationSamples/{}/sample_{}.dat'.format(file_name,
                                                                     i)
            with open(export_name, 'w') as f_write:
                f_write.write(header)
            sample = data_states[:, i, :]
            for j in range(0, nb_timesteps):
                data_list = [str(time_arr[j])] + [str(state) for state in
                                                  sample[j, :]]
        ...
        print(
            'Sampled states exported to AnimationSamples/{}'.format(file_name))
        return None
    ...
    def export_parameters(alg, prior_dict, file_name):
        ...
        print('Learned parameters exported to ParameterLearning/{}'.format(
            file_name))
        return None
