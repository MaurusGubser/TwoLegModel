...
class DataReaderWriter:
    ...
    def read_observations_as_arr(self, path_observations, max_timesteps):
        ...
        for i, j in pairwise(sensor_idxs):
            ...
            df_sensor.columns = ["time"] + [
                col_name + "_" + str(i) for col_name in df_sensor.columns[1:]
            ]
        ...
        if self.observations_arr.shape[0] != self.true_states_arr.shape[0]:
            raise AssertionError(
                ...
                "should be the same.".format(
                    self.observations_arr.shape[0], self.true_states_arr.shape[0]
                )
            )
    ...
    def reduce_observations(self):
        self.observations_arr = self.observations_arr[
            :,
    ...
    def export_trajectory(data_states, dt, file_name):
        ...
        header = (
            header
            + "# time,x_0,x_1,phi femur_l,phi fibula_l,phi femur_r,phi fibula_r,dx_0,dx_1,dphi femur_l,dphi fibula_l,dphi femur_r,dphi fibula_r,ddx_0,ddx_1,ddphi femur_l,ddphi fibula_l,ddphi femur_r,ddphi fibula_r\n"
        )
