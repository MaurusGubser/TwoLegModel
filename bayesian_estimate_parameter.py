...
def set_prior(add_legs, add_imus, add_alphas, set_theta0):
    ...
    if add_legs:
        ...
        if set_theta0:
            theta0 = np.array([(0.5, 0.5, 0.5, 0.5)], dtype=[('femur_left', 'float64'), ('fibula_left', 'float64'),
...
def learn_model_parameters(theta0, prior_dict, structured_prior, learning_alg, Nx, N, t_start, niter, true_states, data,
    ...
    print('Time user {:.0f}h {:.0f}min; time processor {:.0f}h {:.0f}min'.format(s_user // 3600, s_user % 3600 // 60,
                                                                                 s_process // 3600,
                                                                                 s_process % 3600 // 60))
