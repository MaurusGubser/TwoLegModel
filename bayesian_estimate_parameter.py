...
def set_prior(add_Q, add_H, add_legs, add_imus, add_alphas, set_theta0):
    ...
    if add_Q:
        ...
        prior_Q = {
            "scale_x": dists.Uniform(20.0, 200.0),
    ...
    if add_H:
        prior_H = {
            "sigma_imu_acc": dists.LinearD(dists.InvGamma(3.0, 2.0), a=0.1, b=0.0),
        ...
        prior_H = {
            "sigma_imu_acc": dists.Uniform(0.0001, 0.01),
    ...
    if add_legs:
        ...
        if set_theta0:
            theta0 = np.array(
                [(0.5, 0.5)],
                dtype=[("femur_left", "float64"), ("femur_right", "float64")],
            )
    if add_imus:
        prior_imus = {
        ...
        prior_imus = {
            "pos_imu0": dists.Uniform(a=0.0, b=0.5),
            "pos_imu2": dists.Uniform(a=0.0, b=0.5),
        }
        prior_dict.update(prior_imus)
        if set_theta0:
            theta0 = np.array(
                [(0.25, 0.25)], dtype=[("pos_imu0", "float64"), ("pos_imu2", "float64")]
            )
    if add_alphas:
        prior_alphas = {
        ...
        if set_theta0:
            theta0 = np.array(
                [(0.0, 0.0)], dtype=[("alpha_0", "float64"), ("alpha_2", "float64")]
            )
...
def learn_model_parameters(
    ...
    print(
        "Time user {:.0f}h {:.0f}min; time processor {:.0f}h {:.0f}min".format(
            s_user // 3600, s_user % 3600, s_process // 3600, s_process % 3600
        )
    )
...
if __name__ == "__main__":
    ...
    add_alphas = False
    set_theta0 = False
    theta0, prior_dict, prior = set_prior(
        add_Q, add_H, add_legs, add_imu, add_alphas, set_theta0
    )
    Nx = 2000
    N = 20
    t_start = 500
    ...
    export_name = "Learning{}_data{}_steps{}_N{}_niter{}_tstart{}_prior{}".format(
        learning_alg, generation_type, nb_timesteps, N, niter, t_start, prior_str
    )
