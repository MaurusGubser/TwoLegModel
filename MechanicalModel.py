...
def create_rotation_matrix_z(alpha):
    ...
    R = np.array([[np.cos(alpha), np.sin(alpha), 0.0],
                  [-np.sin(alpha), np.cos(alpha), 0.0], [0.0, 0.0, 1.0]])
    return R
...
def state_to_obs(x, dim_observations, g, legs, imus, R):
    ...
    y[:, 0] = imus[0] * x[:, 14] + g * np.sin(x[:, 2]) + np.sin(x[:, 2]) * x[:,
                                                                           13] + np.cos(
        x[:, 2]) * x[:, 12]
    y[:, 1] = imus[0] * x[:, 8] ** 2 + g * np.cos(x[:, 2]) - np.sin(
        x[:, 2]) * x[:, 12] + np.cos(x[:, 2]) * x[:, 13]
    y[:, 2] = 0.0
    y[:, 3] = 0.0
    y[:, 4] = 0.0
    y[:, 5] = x[:, 8]
    ...
    # left fibula
    y[:, 6] = imus[1] * x[:, 14] + imus[1] * x[:, 15] + g * np.sin(
        x[:, 2] + x[:, 3]) + legs[0] * np.sin(
        x[:, 3]) * x[:, 8] ** 2 + legs[0] * np.cos(x[:, 3]) * x[:, 14] + np.sin(
        x[:, 2] + x[:, 3]) * x[:, 13] + np.cos(
        x[:, 2] + x[:, 3]) * x[:, 12]
    y[:, 7] = imus[1] * x[:, 8] ** 2 + 2 * imus[1] * x[:, 8] * x[:, 9] + imus[
        1] * x[:, 9] ** 2 + g * np.cos(
        x[:, 2] + x[:, 3]) - legs[0] * np.sin(x[:, 3]) * x[:, 14] + legs[
                  0] * np.cos(
    ...
    y[:, 12] = imus[2] * x[:, 16] + g * np.sin(x[:, 4]) + np.sin(x[:, 4]) * x[:,
                                                                            13] + np.cos(
        x[:, 4]) * x[:, 12]
    y[:, 13] = imus[2] * x[:, 10] ** 2 + g * np.cos(x[:, 4]) - np.sin(
        x[:, 4]) * x[:, 12] + np.cos(x[:, 4]) * x[:, 13]
    y[:, 14] = 0.0
    y[:, 15] = 0.0
    y[:, 16] = 0.0
    y[:, 17] = x[:, 10]
    ...
    # right fibula
    y[:, 18] = imus[3] * x[:, 16] + imus[3] * x[:, 17] + g * np.sin(
        x[:, 4] + x[:, 5]) + legs[2] * np.sin(
        x[:, 5]) * x[:, 10] ** 2 + legs[2] * np.cos(x[:, 5]) * x[:,
                                                               16] + np.sin(
        x[:, 4] + x[:, 5]) * x[:, 13] + np.cos(
        x[:, 4] + x[:, 5]) * x[:, 12]
    y[:, 19] = imus[3] * x[:, 10] ** 2 + 2 * imus[3] * x[:, 10] * x[:, 11] + \
               imus[3] * x[:, 11] ** 2 + g * np.cos(
        x[:, 4] + x[:, 5]) - legs[2] * np.sin(x[:, 5]) * x[:, 16] + legs[
                   2] * np.cos(x[:, 5]) * x[:, 10] ** 2 - np.sin(
    ...
    y[:, 27] = -legs[0] * np.sin(x[:, 2]) * x[:, 8] ** 2 + legs[0] * np.cos(
        x[:, 2]) * x[:, 14] - legs[1] * (
               ...
               legs[1] * (x[:, 14] + x[:, 15]) * np.cos(
        x[:, 2] + x[:, 3]) + x[:, 12]
    y[:, 28] = legs[0] * np.sin(x[:, 2]) * x[:, 14] + legs[0] * np.cos(
        x[:, 2]) * x[:, 8] ** 2 + legs[1] * (
                       x[:, 8] + x[:, 9]) ** 2 * np.cos(x[:, 2] + x[:, 3]) + \
               legs[1] * (x[:, 14] + x[:, 15]) * np.sin(
        x[:, 2] + x[:, 3]) + x[:, 13]
    y[:, 29] = 0.0
    ...
    y[:, 33] = -legs[2] * np.sin(x[:, 4]) * x[:, 10] ** 2 + legs[2] * np.cos(
        x[:, 4]) * x[:, 16] - legs[3] * (
               ...
               legs[3] * (x[:, 16] + x[:, 17]) * np.cos(
        x[:, 4] + x[:, 5]) + x[:, 12]
    y[:, 34] = legs[2] * np.sin(x[:, 4]) * x[:, 16] + legs[2] * np.cos(
        x[:, 4]) * x[:, 10] ** 2 + legs[3] * (
                       x[:, 10] + x[:, 11]) ** 2 * np.cos(x[:, 4] + x[:, 5]) + \
               legs[3] * (x[:, 16] + x[:, 17]) * np.sin(
        x[:, 4] + x[:, 5]) + x[:, 13]
    y[:, 35] = 0.0
    ...
    if dim_observations == 20:
        y = y[:, (0, 1, 5, 6, 7, 11, 12, 13, 17, 18, 19, 23, 24, 25, 27, 28, 30,
                  31, 33, 34)]
    return y
...
def compute_jacobian_obs(x, dim_states, dim_observations, g, legs, imus, R):
    ...
    df[:, 6, 3] = -x[:, 14] * legs[0] * np.sin(x[:, 3]) - x[:, 12] * np.sin(
        x[:, 2] + x[:, 3]) + x[:, 13] * np.cos(
        x[:, 2] + x[:, 3]) + legs[0] * x[:, 8] ** 2 * np.cos(
        x[:, 3]) + g * np.cos(x[:, 2] + x[:, 3])
    ...
    df[:, 7, 3] = -x[:, 14] * legs[0] * np.cos(x[:, 3]) - x[:, 12] * np.cos(
        x[:, 2] + x[:, 3]) - x[:, 13] * np.sin(
        x[:, 2] + x[:, 3]) - legs[0] * x[:, 8] ** 2 * np.sin(
        x[:, 3]) - g * np.sin(x[:, 2] + x[:, 3])
    ...
    # right femur
    df[:, 12, 4] = -x[:, 12] * np.sin(x[:, 4]) + (x[:, 13] + g) * np.cos(
        x[:, 4])
    df[:, 12, 12] = np.cos(x[:, 4])
    df[:, 12, 13] = np.sin(x[:, 4])
    df[:, 12, 16] = imus[2]
    df[:, 13, 4] = -x[:, 12] * np.cos(x[:, 4]) - (x[:, 13] + g) * np.sin(
        x[:, 4])
    ...
    df[:, 18, 5] = -x[:, 16] * legs[2] * np.sin(x[:, 5]) - x[:, 12] * np.sin(
        x[:, 4] + x[:, 5]) + x[:, 13] * np.cos(
        x[:, 4] + x[:, 5]) + legs[2] * x[:, 10] ** 2 * np.cos(
        x[:, 5]) + g * np.cos(x[:, 4] + x[:, 5])
    ...
    df[:, 19, 5] = -x[:, 16] * legs[2] * np.cos(x[:, 5]) - x[:, 12] * np.cos(
        x[:, 4] + x[:, 5]) - x[:, 13] * np.sin(
        x[:, 4] + x[:, 5]) - legs[2] * x[:, 10] ** 2 * np.sin(
        x[:, 5]) - g * np.sin(x[:, 4] + x[:, 5])
    ...
    df[:, 24, 6] = 1
    df[:, 24, 8] = legs[0] * np.cos(x[:, 2]) + legs[1] * np.cos(
        x[:, 2] + x[:, 3])
    ...
    df[:, 25, 7] = 1
    df[:, 25, 8] = legs[0] * np.sin(x[:, 2]) + legs[1] * np.sin(
        x[:, 2] + x[:, 3])
    df[:, 25, 9] = legs[1] * np.sin(x[:, 2] + x[:, 3])
    df[:, 27, 2] = -legs[0] * (
                x[:, 14] * np.sin(x[:, 2]) + x[:, 8] ** 2 * np.cos(x[:, 2])) - \
                   legs[1] * (
                           x[:, 14] + x[:, 15]) * np.sin(
        x[:, 2] + x[:, 3]) - legs[1] * (x[:, 8] + x[:, 9]) ** 2 * np.cos(
        x[:, 2] + x[:, 3])
    df[:, 27, 3] = -legs[1] * (x[:, 14] + x[:, 15]) * np.sin(
        x[:, 2] + x[:, 3]) - legs[1] * (
                           x[:, 8] + x[:, 9]) ** 2 * np.cos(x[:, 2] + x[:, 3])
    df[:, 27, 8] = -2 * legs[0] * x[:, 8] * np.sin(x[:, 2]) - 2 * legs[1] * (
                x[:, 8] + x[:, 9]) * np.sin(
        x[:, 2] + x[:, 3])
    df[:, 27, 9] = -2 * legs[1] * (x[:, 8] + x[:, 9]) * np.sin(
        x[:, 2] + x[:, 3])
    df[:, 27, 12] = 1
    df[:, 27, 14] = legs[0] * np.cos(x[:, 2]) + legs[1] * np.cos(
        x[:, 2] + x[:, 3])
    df[:, 27, 15] = legs[1] * np.cos(x[:, 2] + x[:, 3])
    df[:, 28, 2] = -legs[0] * (
                -x[:, 14] * np.cos(x[:, 2]) + x[:, 8] ** 2 * np.sin(x[:, 2])) + \
                   legs[1] * (
                           x[:, 14] + x[:, 15]) * np.cos(
        x[:, 2] + x[:, 3]) - legs[1] * (x[:, 8] + x[:, 9]) ** 2 * np.sin(
        x[:, 2] + x[:, 3])
    df[:, 28, 3] = legs[1] * (x[:, 14] + x[:, 15]) * np.cos(x[:, 2] + x[:, 3]) - \
                   legs[1] * (
                           x[:, 8] + x[:, 9]) ** 2 * np.sin(x[:, 2] + x[:, 3])
    ...
    df[:, 28, 13] = 1
    df[:, 28, 14] = legs[0] * np.sin(x[:, 2]) + legs[1] * np.sin(
        x[:, 2] + x[:, 3])
    ...
    df[:, 30, 6] = 1
    df[:, 30, 10] = legs[2] * np.cos(x[:, 4]) + legs[3] * np.cos(
        x[:, 4] + x[:, 5])
    ...
    df[:, 31, 7] = 1
    df[:, 31, 10] = legs[2] * np.sin(x[:, 4]) + legs[3] * np.sin(
        x[:, 4] + x[:, 5])
    df[:, 31, 11] = legs[3] * np.sin(x[:, 4] + x[:, 5])
    df[:, 33, 4] = -legs[2] * (
                x[:, 16] * np.sin(x[:, 4]) + x[:, 10] ** 2 * np.cos(x[:, 4])) - \
                   legs[3] * (
                           x[:, 16] + x[:, 17]) * np.sin(
        x[:, 4] + x[:, 5]) - legs[3] * (x[:, 10] + x[:, 11]) ** 2 * np.cos(
        x[:, 4] + x[:, 5])
    df[:, 33, 5] = -legs[3] * (x[:, 16] + x[:, 17]) * np.sin(
        x[:, 4] + x[:, 5]) - legs[3] * (
                           x[:, 10] + x[:, 11]) ** 2 * np.cos(x[:, 4] + x[:, 5])
    df[:, 33, 10] = -2 * legs[2] * x[:, 10] * np.sin(x[:, 4]) - 2 * legs[3] * (
                x[:, 10] + x[:, 11]) * np.sin(
        x[:, 4] + x[:, 5])
    df[:, 33, 11] = -2 * legs[3] * (x[:, 10] + x[:, 11]) * np.sin(
        x[:, 4] + x[:, 5])
    df[:, 33, 12] = 1
    df[:, 33, 16] = legs[2] * np.cos(x[:, 4]) + legs[3] * np.cos(
        x[:, 4] + x[:, 5])
    df[:, 33, 17] = legs[3] * np.cos(x[:, 4] + x[:, 5])
    df[:, 34, 4] = -legs[2] * (
                -x[:, 16] * np.cos(x[:, 4]) + x[:, 10] ** 2 * np.sin(x[:, 4])) + \
                   legs[3] * (
                           x[:, 16] + x[:, 17]) * np.cos(
        x[:, 4] + x[:, 5]) - legs[3] * (x[:, 10] + x[:, 11]) ** 2 * np.sin(
        x[:, 4] + x[:, 5])
    df[:, 34, 5] = legs[3] * (x[:, 16] + x[:, 17]) * np.cos(x[:, 4] + x[:, 5]) - \
                   legs[3] * (
                           x[:, 10] + x[:, 11]) ** 2 * np.sin(x[:, 4] + x[:, 5])
    df[:, 34, 10] = 2 * legs[2] * x[:, 10] * np.cos(x[:, 4]) + 2 * legs[3] * (
                x[:, 10] + x[:, 11]) * np.cos(
        x[:, 4] + x[:, 5])
    df[:, 34, 11] = 2 * legs[3] * (x[:, 10] + x[:, 11]) * np.cos(
        x[:, 4] + x[:, 5])
    df[:, 34, 13] = 1
    df[:, 34, 16] = legs[2] * np.sin(x[:, 4]) + legs[3] * np.sin(
        x[:, 4] + x[:, 5])
    ...
    if dim_observations == 20:
        df = df[:, (0, 1, 5, 6, 7, 11, 12, 13, 17, 18, 19, 23, 24, 25, 27, 28,
                    30, 31, 33, 34), :]
    return df
...
def state_to_obs_linear(x, xp, dim_states, dim_observations, g, legs, imus, R):
    ...
    if xp is None:
        xp = np.zeros(x.shape)
    nb_steps, _ = x.shape
    df = compute_jacobian_obs(xp, dim_states, dim_observations, g, legs, imus,
                              R)
...
class MechanicalModel:
    def __init__(self, dt, dim_states, dim_observations, imu_position,
                 leg_constants, R):
        self.dt = dt
    ...
    def compute_jacobian_observation(self, x):
        return compute_jacobian_obs(x, self.dim_states, self.dim_observations,
                                    self.g, self.leg_len, self.imu_pos,
                                    self.R)
    ...
    def compute_jacobian_observation_numeric(self, x, delta_x=0.01):
        ...
        for j in range(self.dim_states):
            ...
            x_plus[:, j] += delta_x
            x_minus[:, j] -= delta_x
            delta_f = self.state_to_observation(
                x_plus) - self.state_to_observation(x_minus)
            df[:, j] = delta_f / (2 * delta_x)
        return df

    def state_to_observation_linear(self, x, xp):
        return state_to_obs_linear(x, xp, self.dim_states,
                                   self.dim_observations, self.g, self.leg_len,
                                   self.imu_pos,
                                   self.R)
