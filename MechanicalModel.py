...
def state_to_obs(x, dim_observations, g, legs, imus, R):
    ...
    # left heel
    y[:, 24] = (
        legs[0] * np.cos(x[:, 2]) * x[:, 8]
        + legs[1] * (x[:, 8] + x[:, 9]) * np.cos(x[:, 2] + x[:, 3])
        + x[:, 6]
    )
    y[:, 25] = (
        legs[0] * np.sin(x[:, 2]) * x[:, 8]
        + legs[1] * (x[:, 8] + x[:, 9]) * np.sin(x[:, 2] + x[:, 3])
        + x[:, 7]
    )
    y[:, 26] = 0.0
    y[:, 27] = (
    ...
    # right heel
    y[:, 30] = (
        legs[2] * np.cos(x[:, 4]) * x[:, 10]
        + legs[3] * (x[:, 10] + x[:, 11]) * np.cos(x[:, 4] + x[:, 5])
        + x[:, 6]
    )
    y[:, 31] = (
        legs[2] * np.sin(x[:, 4]) * x[:, 10]
        + legs[3] * (x[:, 10] + x[:, 11]) * np.sin(x[:, 4] + x[:, 5])
        + x[:, 7]
    )
    y[:, 32] = 0.0
    y[:, 33] = (
...
class MechanicalModel:
    def __init__(
        self, dt, dim_states, dim_observations, imu_position, leg_constants, R
    ):
        self.dt = dt
    ...
    def state_to_observation(self, x):
        return state_to_obs(
            x, self.dim_observations, self.g, self.leg_len, self.imu_pos, self.R
        )
